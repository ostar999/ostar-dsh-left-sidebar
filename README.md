...
<div align="center">
  ...
  <img alt="version" src="https://img.shields.io/badge/version-0.2.0-4d6bfe" />
...
- **删除**:调用官方客户端服务 `ctx.workspaces.delete(workspaceId)` 与 `ctx.workspaces.archiveSession(sessionId)` —— 与官方浏览器完全相同,store 自动同步。删除语义与官方一致:工作区删除 = 注册移除(会话落入「未分组」);会话删除 = 归档(从分组面隐藏,日志保留)。
...
ostar-dsh-left-sidebar/
├── src/
│   ├── index.js          # Host 半区:迁移/复制路由 + 分组持久化路由
...
- **删除语义与官方一致**:工作区删除只移除注册(其会话进入「未分组」),会话删除是**归档**而非物理删除,日志仍保留在 `~/.dsh/sessions/` 下;本插件不提供物理删除。
